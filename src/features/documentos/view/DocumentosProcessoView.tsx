...
export function DocumentosProcessoView({ params }: { params: Promise<{ id: string }> }): React.ReactNode {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    if (files && files.length > 0) {
      ...
      if (file.size > 25 * 1024 * 1024) {
        setFileError("O arquivo excede o limite máximo de 25 MB.");
        setSelectedFile(null);
        return;
      }
  ...
  return (
    <main className="flex min-h-screen flex-col items-center bg-slate-50 px-4 py-12 md:px-8">
      <div className="w-full max-w-4xl">
        ...
        {isLoading && !processo ? (
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl shadow-sm border border-slate-100">
            <div className="h-8 w-8 animate-spin rounded-full border-4 border-slate-300 border-t-[#1a3c5e]"></div>
            <p className="mt-4 text-sm text-slate-500 font-medium">Carregando processo...</p>
          </div>
        ) : !processo ? (
          <div className="rounded-2xl bg-white p-8 text-center shadow-sm border border-slate-100">
            <p className="text-slate-500">Processo não encontrado ou erro ao carregar.</p>
          ...
          <div className="space-y-8">
            ...
            <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Processo Vinculado</span>
                <h1 className="text-2xl font-bold text-slate-900 mt-1">{processo.numero}</h1>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold bg-slate-50 text-slate-700 border border-slate-200 uppercase">
                  {processo.tipo}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-1 space-y-6">
                <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
                  <h2 className="text-lg font-bold text-slate-800 mb-4">Anexar Novo PDF</h2>
                  ...
                  <form onSubmit={(e) => { void handleSubmit(onSubmit)(e); }} noValidate className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="relative border-2 border-dashed border-slate-200 rounded-lg p-4 text-center hover:border-[#1a3c5e]/50 transition-colors">
                        <input
                          id="file-upload"
                          type="file"
                          accept=".pdf"
                        ...
                        <div className="space-y-1">
                          ...
                          <span className="block text-[10px] text-slate-400">PDF de até 25MB</span>
                        </div>
                      </div>
                    ...
                    {/* Descrição opcional */}
                    <div>
                      <label htmlFor="descricao" className="block text-sm font-semibold text-slate-600 mb-1">
                        Descrição (opcional)
                      </label>
                      <input
                        id="descricao"
                        ...
                        className={`
                          ...
                          ${errors.descricao ? "border-red-400 focus:border-red-400" : "border-slate-200"}
                        `}
                      />
              ...
              <div className="lg:col-span-2 space-y-4">
                <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
                  <h2 className="text-lg font-bold text-slate-800 mb-4">Documentos Anexados</h2>

                  {processo.documentos.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12 text-center">
                      <span className="text-4xl mb-3 text-slate-300">folder_open</span>
                      <p className="text-sm font-semibold text-slate-500">Nenhum documento anexado</p>
                      <p className="text-xs text-slate-400 mt-1">Utilize o painel lateral para anexar o primeiro arquivo PDF.</p>
                    </div>
                  ) : (
                      ...
                      {processo.documentos.map((doc) => (
                        <div key={doc.id} className="py-4 first:pt-0 last:pb-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                          <div className="space-y-1 min-w-0">
                            <h3 className="text-sm font-semibold text-slate-900 truncate" title={doc.nomeArquivo}>
                              {doc.nomeArquivo}
                            </h3>
                            {doc.descricao && (
                          ...
                          <div className="flex flex-col sm:flex-row items-center gap-2 mt-3 sm:mt-0">
                            <a
                              ...
                              className="
                                inline-flex items-center justify-center rounded-lg border border-slate-200
